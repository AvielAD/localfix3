...
const Add = (params: { values:DevicesAssignDto | null, setValues: (params: DevicesAssignInputDto) => void}) => {
    ...
    const submitAdd = async (values: DevicesAssignFormDto, { resetForm }: any) => {
        ...
        let dataupdate:DevicesAssignInputDto = {
            ...
            cost1: values.cost1 === "" ? 0 : parseFloat(values.cost1),
            cost2: values.cost2 === "" ? 0 :parseFloat(values.cost2),
            cost3: values.cost3 === "" ? 0 :parseFloat(values.cost3),
            costSale: parseFloat(values.costSale),
                    ...
                    <Form className="">
                        <div className="grid grid-cols-1 px-4 py-1 mb-8 bg-white rounded-lg shadow-md dark:bg-secondary-800">
                            ...
                            <label className="block text-sm">
                                <span className="text-secondary-700 dark:text-secondary-400">Costo 1</span>
                                <Field
                                    name="cost1"
                            ...
                            <label className="block text-sm">
                                <span className="text-secondary-700 dark:text-secondary-400">Costo 2</span>
                                <Field
                                    name="cost2"
                                    className={`shadow appearance-none border block w-full mt-1 text-sm dark:border-secondary-600 dark:bg-secondary-700 focus:border-theme3-400 focus:outline-none focus:shadow-outline-theme3 dark:text-secondary-300 dark:focus:shadow-outline-secondary form-input focus:ring-2 focus:ring-primary-600`}
                                ></Field>
                            ...
                            <label className="block text-sm">
                                <span className="text-secondary-700 dark:text-secondary-400">Costo 3</span>
                                <Field
                                    name="cost3"
                                    className={`shadow appearance-none border block w-full mt-1 text-sm dark:border-secondary-600 dark:bg-secondary-700 focus:border-theme3-400 focus:outline-none focus:shadow-outline-theme3 dark:text-secondary-300 dark:focus:shadow-outline-secondary form-input focus:ring-2 focus:ring-primary-600`}
                                ></Field>
                            ...
                            <label className="block text-sm">
                                <span className="text-secondary-700 dark:text-secondary-400">Costo Venta</span>
                                <Field
                                    name="costSale"
                                    className={`shadow appearance-none border block w-full mt-1 text-sm dark:border-secondary-600 dark:bg-secondary-700 focus:border-theme3-400 focus:outline-none focus:shadow-outline-theme3 dark:text-secondary-300 dark:focus:shadow-outline-secondary form-input focus:ring-2 focus:ring-primary-600`}
                                ></Field>
...
const addSchema = object({
    ...
    costSale: number().min(1, 'El valor no puede ser negativo').required('Campo Requerido').typeError("Valor Incorrecto"),
})
